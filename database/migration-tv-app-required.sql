-- ============================================================
-- TV uygulaması için ZORUNLU migration
-- Uygulama "broadcast_code sütunu veya ekran tablosu eksik" derse bu dosyayı çalıştırın.
-- Supabase: Dashboard → SQL Editor → New query → bu dosyanın içeriğini yapıştır → Run
-- ============================================================

-- 1) screens.broadcast_code (5 haneli yayın kodu)
...
-- 2) TV uygulaması ayarları tablosu (Admin → Ayarlar)
...
-- 3) Sürüm yönetimi sütunları (isteğe bağlı)
ALTER TABLE tv_app_settings ADD COLUMN IF NOT EXISTS min_version_code INTEGER;
ALTER TABLE tv_app_settings ADD COLUMN IF NOT EXISTS latest_version_code INTEGER;
ALTER TABLE tv_app_settings ADD COLUMN IF NOT EXISTS latest_version_name TEXT;

-- Bitti. Admin → Ekranlar'da "TV uygulaması yayın kodu" görünmeli; uygulamada bu kodu girin.
