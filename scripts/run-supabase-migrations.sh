...
echo "[3/6] allow_multi_device (super admin çoklu cihaz)..."
psql -h "$SUPABASE_DB_HOST" -p 5432 -U postgres -d postgres \
  -f "${PROJECT_ROOT}/database/migration-add-allow-multi-device.sql" -v ON_ERROR_STOP=0
echo "  Tamamlandı."
echo ""

echo "[4/6] Import sütunları + registration_requests..."
psql -h "$SUPABASE_DB_HOST" -p 5432 -U postgres -d postgres \
...
echo "[5/6] TV uygulaması (broadcast_code + tv_app_settings)..."
psql -h "$SUPABASE_DB_HOST" -p 5432 -U postgres -d postgres \
  -f "${PROJECT_ROOT}/database/migration-supabase-tv-app.sql" -v ON_ERROR_STOP=0
echo "  Tamamlandı."
echo ""

echo "[6/6] migrations/ klasörü..."
