...
export default function LocaleAdminError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    try {
      if (error) console.error('[locale-admin]', error?.message ?? String(error));
    } catch {
      // Hata loglanırken bile hata oluşabilir (örn. circular reference)
    }
  }, [error]);
