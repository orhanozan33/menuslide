...
export default function EditorError({
  ...
  useEffect(() => {
    try {
      if (error) console.error('[editor]', error?.message ?? String(error));
    } catch {
      /* ignore */
    }
  }, [error]);

  const message = error?.message || 'Editör başlatılırken bir hata oluştu. Sayfayı yenileyin veya daha sonra tekrar deneyin.';
