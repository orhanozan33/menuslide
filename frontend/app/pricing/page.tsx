...
function PricingPageContent() {
  ...
  const loadData = async () => {
    try {
      ...
      const raw = Array.isArray(plansData)
        ? plansData
        : (plansData && typeof plansData === 'object' && (Array.isArray((plansData as any).data) ? (plansData as any).data : Array.isArray((plansData as any).plans) ? (plansData as any).plans : []));
      const allowedMax = [3, 5, 7, 10, -1];
      const filtered = Array.isArray(raw)
        ? raw.filter((p: Plan) => p.is_active !== false && allowedMax.includes(Number(p.max_screens)))
        : [];
      setPlans(filtered.length > 0 ? filtered : (Array.isArray(raw) ? raw.filter((p: Plan) => p.is_active !== false) : []));
