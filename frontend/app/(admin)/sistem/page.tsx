...
export default function SistemPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const saveDraft = () => {
        ...
        if (!c) return;
        try {
          const json = (c as { toObject: (p?: string[]) => object }).toObject(['selectable', 'evented', 'fontFamily', 'fontWeight', 'fontStyle', 'fontSize', 'fill']);
    ...
    const pushHistory = () => {
      ...
      if (!c) return;
      try {
        const json = JSON.stringify((c as { toObject: (p?: string[]) => object }).toObject(['selectable', 'evented', 'fontFamily', 'fontWeight', 'fontStyle', 'fontSize', 'fill']));
  ...
  const newCanvas = useCallback(async () => {
    ...
    const json = JSON.stringify((canvas as { toObject: (p?: string[]) => object }).toObject(['selectable', 'evented', 'fontFamily', 'fontWeight', 'fontStyle', 'fontSize', 'fill']));
      ...
      if (savedTemplateId && isSameName) {
        ...
        const res = await fetch('/api/full-editor/templates', {
          ...
          body: JSON.stringify({
            id: savedTemplateId,
            name: name.trim(),
            canvas_json: (canvas as { toObject: (p?: string[]) => object }).toObject(['selectable', 'evented', 'fontFamily', 'fontWeight', 'fontStyle', 'fontSize', 'fill']),
