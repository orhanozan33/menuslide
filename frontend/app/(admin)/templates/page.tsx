                          ...
                          if (m === 'middle_both') {
                            return (
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  type M = 'left' | 'middle' | 'right' | 'middle_left' | 'middle_right' | 'middle_2_as_one';
                                  setCreateSystemMergeOptions((p) => {
                                    const cur: M[] = p[blockCount] || [];
                                    if (middleBothActive) {
                                      return { ...p, [blockCount]: cur.filter((x): x is M => x !== 'middle_left' && x !== 'middle_right') };
                                    }
                                    const next: M[] = [...cur.filter((x) => x !== 'middle_left' && x !== 'middle_right' && x !== 'middle_2_as_one'), 'middle_left', 'middle_right'];
                                ...
                                onClick={() => {
                                  ...
                                  setCreateSystemMergeOptions((p) => {
                                    ...
                                    if (middle2AsOneActive) {
                                      return { ...p, [blockCount]: cur.filter((x): x is M => x !== 'middle_2_as_one') };
                                    }
                                    const next: M[] = [...cur.filter((x) => x !== 'middle_left' && x !== 'middle_right' && x !== 'middle_2_as_one'), 'middle_2_as_one'];
                                    return { ...p, [blockCount]: next };
                                  });
                                }}
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setCreateSystemMergeOptions((p) => {
                                  const cur: M[] = p[blockCount] || [];
                                  const next: M[] = active ? cur.filter((x): x is M => x !== m) : [...cur.filter((x) => x !== 'middle_2_as_one'), m as M];
                                  return { ...p, [blockCount]: next };
                                });
                              }}
